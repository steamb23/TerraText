...
namespace TerraText
{
    ...
    public static class UnicodeWidth
    {
        ...
        static readonly Interval[] combining = new Dictionary<int, int> {
        ...
        /// <summary>
        /// 현재 모드가 CJK 모드인지 여부에 대한 값을 설정하거나 가져옵니다.
        /// </summary>
        /// <remarks>일반적으로 영어권 글꼴은 비 CJK 모드로 정확한 길이를 확인할 수 있지만 굴림체, 돋움체, MS Gothic 등의 오래된 문자열은 CJK 모드로 길이를 확인해야합니다.</remarks>
        public static bool IsModeCJK { get; set; }

        /// <summary>
        /// 현재 설정된 CJK 모드에서의 글자 폭을 가져옵니다.
        /// </summary>
        /// <param name="character">글자 폭을 확인할 글자입니다.</param>
        /// <returns>전각은 2, 반각은 1로 표현되며 제어 문자는 -1, 그외 공백은 0입니다.</returns>
        public static int GetWidth(char character) => IsModeCJK ? GetWidthCJK(character) : GetWidthNonCJK(character);

        /// <summary>
        /// 현재 설정된 CJK 모드에서의 문장 길이를 가져옵니다.
        /// </summary>
        /// <param name="text">문장 길이를 확인할 문자입니다.</param>
        /// <returns>전각 길이, 반각 길이가 반영된 문장 길이입니다.</returns>
        public static int GetWidth(string text) => IsModeCJK ? GetWidthCJK(text) : GetWidthNonCJK(text);

        /// <summary>
        /// 비 CJK 모드에서의 글자 폭을 가져옵니다.
        /// </summary>
        /// <param name="character">글자 폭을 확인할 글자입니다.</param>
        /// <returns>전각은 2, 반각은 1로 표현되며 제어 문자는 -1, 그외 공백은 0입니다.</returns>
        public static int GetWidthNonCJK(char character)
        {
        ...
        static readonly Interval[] ambiguous = new Dictionary<int, int> {
        ...
        /*
            ...
            */
        /// <summary>
        /// CJK 모드에서의 글자 폭을 가져옵니다.
        /// </summary>
        /// <param name="character">글자 폭을 확인할 글자입니다.</param>
        /// <returns>전각은 2, 반각은 1로 표현되며 제어 문자는 -1, 그외 공백은 0입니다.</returns>
        public static int GetWidthCJK(char character)
        {
            /* binary search in table of non-spacing characters */
            if (BinarySearch(character, ambiguous))
                return 2;

            return GetWidthNonCJK(character);
        }

        /// <summary>
        /// CJK 모드에서의 문장 길이를 가져옵니다.
        /// </summary>
        /// <param name="text">문장 길이를 확인할 문자입니다.</param>
        /// <returns>전각 길이, 반각 길이가 반영된 문장 길이입니다.</returns>
